...
namespace csc424_se2_organApp.Controllers
{
    ...
    [ApiController]
    public class DonLiveController : Controller{


        private readonly organ_appContext context;
        /// <summary>Reference database context</summary>
        public DonLiveController(organ_appContext _context)
        {
            context = _context;
        }

        [HttpPost]
        public JsonResult UpdateRecord([FromBody]DonorLive input){
            
            try{
                context.DonorLive.Update(input);
                context.SaveChanges();
        ...
        public JsonResult GetRecordPersId([FromBody]dynamic input){
            ...
            var isInDb = context.DonorLive.Where(r => r.PersId == num)
                                        .FirstOrDefault<DonorLive>();
            if(isInDb == null){
        ...
        public IActionResult DownloadRecord([FromBody] dynamic input){
            ...
            foreach(var a in input.DonorId){
                num = a;
                var isInDb = context.DonorLive.Where(r => r.DonorId == num)
                                            .FirstOrDefault<DonorLive>();
                if(isInDb == null){
                    Response.StatusCode = 404;
        ...
        public JsonResult GetRecordByState([FromBody]dynamic input){
            ...
            var query = (from c in context.DonorLive 
        ...
        public JsonResult SearchRecordPersIdFirstX([FromBody]dynamic input){
            ...
            var isInDb = (from c in context.DonorLive
